#!/usr/bin/env python3
"""Генерирует 40 разных видео-режимов (SFW) в config/modes/video/.

Каждый режим = i2v анимация исходного фото с СОХРАНЕНИЕМ лица/идентичности/локации
и своим движением. Контент-менеджер правит только prompt_template/negative/params.
Перегенерация безопасна — перезаписывает VIDEO_VARIATION_1..40.
"""
import os

OUT = os.path.join(os.path.dirname(__file__), "..", "config", "modes", "video")

# (номер, краткое название, движение для промта)
MODES = [
    # --- Камера (1-10) ---
    ...
    (33, "dreamy_soft",    "dreamy soft-focus drift, gentle bokeh"),
...
for num, title, motion in MODES:
    mode_id = f"VIDEO_VARIATION_{num}"
    content = TEMPLATE.format(id=mode_id, title=title, motion=motion)
    path = os.path.join(OUT, f"{mode_id}.yaml")
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(content)
print(f"Wrote {len(MODES)} video modes to {os.path.normpath(OUT)}")
