...
## 9b. Партнёрский биллинг (оплата за генерацию)

Партнёры (каждый = API-ключ) платят **предоплатой**: баланс в кредитах (1 кредит = 1 ₽),
списание за каждую генерацию, при нуле — отказ 402, при сбое — автовозврат. Цена по умолчанию
**40 ₽/видео** (`config/pricing.yaml`). Включается `BILLING_ENABLED=true` в `.env`.
- Пополнить баланс партнёра: `python -m scripts.topup <api_key_id> <кредиты>` (или `POST /admin/billing/{id}/topup`).
- Партнёр смотрит: `GET /billing/balance`, `GET /billing/usage`.
- Подробно: [VIDEO_API_INTEGRATION.md](VIDEO_API_INTEGRATION.md) §8b.
